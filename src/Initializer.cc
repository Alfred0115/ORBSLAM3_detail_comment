    ...
    const vector<cv::Point2f> &vP2) //归一化后的点, in current frame
{
    ...
    //     为什么H的解是与最小奇异值对应的右奇异向量（V矩阵的最后一列）：

    // 在使用奇异值分解（SVD）求解单应性矩阵（Homography）时，
    // 选择与最小奇异值对应的右奇异向量（即V矩阵的最后一列）
    // 作为解的原因与线性方程组的性质和SVD的数学特性有关。
    //https://blog.csdn.net/weixin_47180908/article/details/135943606
