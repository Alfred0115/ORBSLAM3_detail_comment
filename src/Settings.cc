...
Settings::Settings(const std::string &configFile, const int &sensor) : bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false)
{
    ...
    cout << "Settings-->> "<<"\t-Loaded camera 1" << endl;

    // Read second camera if stereo (not rectified)
    if (sensor_ == System::STEREO || sensor_ == System::IMU_STEREO)
    {
        readCamera2(fSettings);
        cout << "Settings-->> "<<"\t-Loaded camera 2" << endl;
    }

    // Read image info
    readImageInfo(fSettings);
    cout << "Settings-->> "<<"\t-Loaded image info" << endl;

    if (sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_STEREO || sensor_ == System::IMU_RGBD)
    {
        readIMU(fSettings);
        cout << "Settings-->> "<<"\t-Loaded IMU calibration" << endl;
    }

    if (sensor_ == System::RGBD || sensor_ == System::IMU_RGBD)
    {
        readRGBD(fSettings);
        cout << "Settings-->> "<<"\t-Loaded RGB-D calibration" << endl;
    }

    readORB(fSettings);
    cout << "Settings-->> "<<"\t-Loaded ORB settings" << endl;
    readViewer(fSettings);
    cout << "Settings-->> "<<"\t-Loaded viewer settings" << endl;
    readLoadAndSave(fSettings);
    cout << "Settings-->> "<<"\t-Loaded Atlas settings" << endl;
    readOtherParameters(fSettings);
    cout << "Settings-->> "<<"\t-Loaded misc parameters" << endl;

    if (bNeedToRectify_)
    {
        precomputeRectificationMaps();
        cout << "Settings-->> "<<"\t-Computed rectification maps" << endl;
    }
